...
zone_object.onZoneIn = function(player, prevZone)
    ...
    if player:getCurrentMission(WINDURST) == tpz.mission.id.windurst.FULL_MOON_FOUNTAIN and player:getMissionStatus(player:getNation()) == 3 then
        cs = 50
    elseif player:getCurrentMission(WINDURST) == tpz.mission.id.windurst.DOLL_OF_THE_DEAD and player:getMissionStatus(player:getNation()) == 7 then
        cs = 61
    end
    return cs
end
