...
entity.onTrigger = function(player, npc)
    ...
    if (CurrentMission == tpz.mission.id.sandoria.THE_DAVOI_REPORT and player:getMissionStatus(player:getNation()) == 1) then
        player:setMissionStatus(player:getNation(), 2)
        player:addKeyItem(tpz.ki.LOST_DOCUMENT)
        player:messageSpecial(ID.text.KEYITEM_OBTAINED, tpz.ki.LOST_DOCUMENT)
    elseif (CurrentMission == tpz.mission.id.sandoria.INFILTRATE_DAVOI and player:getMissionStatus(player:getNation()) >= 6 and player:getMissionStatus(player:getNation()) <= 9) then
        local X = npc:getXPos()
        local Z = npc:getZPos()
