...
entity.onTrigger = function(player, npc)
    ...
    if currentMission == tpz.mission.id.windurst.A_NEW_JOURNEY and player:getMissionStatus(player:getNation()) == 2 and not player:hasKeyItem(tpz.ki.DELKFUTT_KEY) then
        player:messageSpecial(ID.text.THE_DOOR_IS_FIRMLY_SHUT_OPEN_KEY)
    elseif currentMission == tpz.mission.id.windurst.A_NEW_JOURNEY and player:getMissionStatus(player:getNation()) == 2 and player:hasKeyItem(tpz.ki.DELKFUTT_KEY) then
        player:startEvent(2)
    else
