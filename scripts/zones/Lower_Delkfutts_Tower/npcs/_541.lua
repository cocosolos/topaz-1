...
entity.onTrigger = function(player, npc)
    ...
    if currentMission == tpz.mission.id.sandoria.APPOINTMENT_TO_JEUNO and player:getMissionStatus(player:getNation()) == 4 and not player:hasKeyItem(tpz.ki.DELKFUTT_KEY) then
        player:messageSpecial(ID.text.THE_DOOR_IS_FIRMLY_SHUT_OPEN_KEY)
    elseif currentMission == tpz.mission.id.sandoria.APPOINTMENT_TO_JEUNO and player:getMissionStatus(player:getNation()) == 4 and player:hasKeyItem(tpz.ki.DELKFUTT_KEY) then
        player:startEvent(0)
    else
