...
zone_object.onZoneIn = function(player, prevZone)
    ...
    if player:getCurrentMission(ZILART) == tpz.mission.id.zilart.RETURN_TO_DELKFUTTS_TOWER and player:getMissionStatus(tpz.mission.log_id.ZILART) <= 1 then
        cs = 15
...
zone_object.onEventFinish = function(player, csid, option)
    if csid == 15 then
        player:setMissionStatus(tpz.mission.log_id.ZILART, 2)
