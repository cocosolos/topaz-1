...
zone_object.onZoneIn = function(player, prevZone)
    ...
    if (player:getCurrentMission(ZILART) == tpz.mission.id.zilart.RETURN_TO_DELKFUTTS_TOWER and player:getMissionStatus(tpz.mission.log_id.ZILART) >= 0) then
        cs = 0
    end
...
zone_object.onEventFinish = function(player, csid, option)
    ...
    elseif (csid == 0) then
        player:setMissionStatus(tpz.mission.log_id.ZILART, 3)
    end
