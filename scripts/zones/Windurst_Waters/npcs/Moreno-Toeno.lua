...
entity.onTrigger = function(player, npc)
    ...
    if (player:getCurrentMission(WINDURST) == tpz.mission.id.windurst.VAIN and player:getMissionStatus(player:getNation()) == 0) then
        player:startEvent(752, 0, tpz.ki.STAR_SEEKER)
    elseif (player:getCurrentMission(WINDURST) == tpz.mission.id.windurst.VAIN and player:getMissionStatus(player:getNation()) >= 1) then
