...
zone_object.onZoneIn = function(player, prevZone)
    ...
    if (player:getCurrentMission(ZILART) == tpz.mission.id.zilart.THE_NEW_FRONTIER and player:getRank(player:getNation()) >= 6) then
        cs = 1
    elseif (player:getCurrentMission(ZILART) == tpz.mission.id.zilart.AWAKENING and player:getMissionStatus(tpz.mission.log_id.ZILART) == 0 or player:getMissionStatus(tpz.mission.log_id.ZILART) == 2) then
        cs = 176
    end
