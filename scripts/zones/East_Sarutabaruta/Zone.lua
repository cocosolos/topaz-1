...
zone_object.onZoneIn = function(player, prevZone)
    ...
    if (player:getCurrentMission(WINDURST) == tpz.mission.id.windurst.THE_HEART_OF_THE_MATTER and
        player:getMissionStatus(player:getNation()) == 5 and prevZone == tpz.zone.OUTER_HORUTOTO_RUINS) then
        cs = 48;
    elseif quests.rainbow.onZoneIn(player) then
        cs = 50;
    elseif (player:getCurrentMission(WINDURST) == tpz.mission.id.windurst.VAIN and player:getMissionStatus(player:getNation()) ==
        1) then
