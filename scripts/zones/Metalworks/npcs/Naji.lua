...
entity.onTrigger = function(player, npc)
    ...
    elseif (player:getCurrentMission(BASTOK) ~= tpz.mission.id.bastok.NONE) then
        ...
        elseif (currentMission == tpz.mission.id.bastok.THE_EMISSARY) then
            if (player:hasKeyItem(tpz.ki.LETTER_TO_THE_CONSULS_BASTOK) == false and player:getMissionStatus(player:getNation()) ==
                0) then
                player:startEvent(713)
            else
                player:showText(npc, ID.text.GOOD_LUCK)
            end
        elseif (player:hasKeyItem(tpz.ki.MESSAGE_TO_JEUNO_BASTOK) and player:getMissionStatus(player:getNation()) == 0) then
            player:startEvent(720)
        elseif (currentMission == tpz.mission.id.bastok.DARKNESS_RISING and player:getMissionStatus(player:getNation()) == 1) then
            player:startEvent(721)
        elseif (player:hasKeyItem(tpz.ki.BURNT_SEAL)) then
            player:startEvent(722)
        elseif (currentMission == tpz.mission.id.bastok.THE_PIRATE_S_COVE and player:getMissionStatus(player:getNation()) == 0) then
            player:startEvent(761)
        elseif (currentMission == tpz.mission.id.bastok.THE_PIRATE_S_COVE and player:getMissionStatus(player:getNation()) == 3) then
