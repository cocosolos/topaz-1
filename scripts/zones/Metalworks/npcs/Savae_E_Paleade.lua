...
entity.onTrade = function(player, npc, trade)
    ...
    if (player:getCurrentMission(SANDORIA) == tpz.mission.id.sandoria.JOURNEY_TO_BASTOK and player:getMissionStatus(player:getNation()) == 5) then
...
entity.onTrigger = function(player, npc)
    ...
    if (player:getCurrentMission(SANDORIA) == tpz.mission.id.sandoria.JOURNEY_ABROAD and player:getMissionStatus(player:getNation()) == 2) then
        player:startEvent(204)
    -- San d'Oria Mission 2-3 Part II - Windurst > Bastok
    elseif (player:getCurrentMission(SANDORIA) == tpz.mission.id.sandoria.JOURNEY_ABROAD and player:getMissionStatus(player:getNation()) == 7) then
        player:startEvent(206)
    elseif (player:getCurrentMission(SANDORIA) == tpz.mission.id.sandoria.JOURNEY_TO_BASTOK2 and player:getMissionStatus(player:getNation()) == 11) then
