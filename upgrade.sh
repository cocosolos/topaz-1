...
then
    ...
    # Set zoneip
    cd ..
    TOPAZ_LOGIN=`grep -oP 'mysql_login:\s+\K\w+' conf/map.conf`
    TOPAZ_PASSWORD=`grep -oP 'mysql_password:\s+\K\w+' conf/map.conf`
    TOPAZ_DB=`grep -oP 'mysql_database:\s+\K\w+' conf/map.conf`
    ...
    then
        ...
        if [ ! -z "$IP" ]
        then
            mysql -u $TOPAZ_LOGIN -p$TOPAZ_PASSWORD $TOPAZ_DB -e "UPDATE zone_settings SET zoneip = '$IP';"
    ...
    #then
    #    mysql -u $TOPAZ_LOGIN -p$TOPAZ_PASSWORD $TOPAZ_DB < sql/backups/custom.sql
    #fi
